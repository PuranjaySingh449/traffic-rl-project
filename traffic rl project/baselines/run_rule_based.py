...
OUTPUT_CSV = os.path.join(BASE_DIR, "..", "logs", "rule_based_metrics.csv")

GREEN_EXTEND = 15  # seconds
...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        # RULE: if emergency present → extend green
        if emergency_present:
            tls_id = traci.trafficlight.getIDList()[0]
            traci.trafficlight.setPhaseDuration(tls_id, GREEN_EXTEND)
